...
namespace kvantum
{
   ...
   struct TypeNode
   {
      ...
      TypeNode(string n) : name(n), fields(), methods(){}
      TypeNode* copy(string newID = "")
      {
          if (newID == "")
              newID = name;
          auto node = new TypeNode(newID);
          std::for_each(ITER_THROUGH(fields), [&node](std::pair<string, Type*> f) { node->fields.emplace(f.first,f.second); });
          std::for_each(ITER_THROUGH(methods), [&node](std::pair<string, FunctionNode*> f) { node->methods.emplace(f.first, f.second); });
          return node;
      }
   };
   ...
   public:
      ...
      bool hasFunction(string name) { return node->methods.count(name) + (parent ? parent->hasFunction(name) : 0) > 0; }
   ...
   public:
      ...
      unsigned int getAllocSize() override { return 4; }
      bool equals(Type& other) const override { return other.isArray() && other.asArray().getType() == type; }
