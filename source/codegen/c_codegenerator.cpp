...
namespace kvantum::codegen
{
   C_Generator::C_Generator() : generator()
   {
      ...
      primitiveTypes[PrimitiveType::Rational] = c::ast::Type::getDouble();
   ...
   void C_Generator::generateFunction(FunctionNode* func)
   {
      ...
      auto f = generator.createFunction(func->getID(),getCType(func->returnType));
      for(auto &e : func->formalParams){
         f->formalParams.push_back(new c::ast::Variable(e->id,getCType(e->type)));
      }
      for(auto &e : func->ast){
         visit_statement(e);
      }
   ...
   {
      c::ast::Expression* generated = new c::ast::Variable(var->id,getCType(var->type));
      if (var->isField()) {
   ...
   void C_Generator::visit(Assigment* assig)
   {
      ...
      generator.createAssignment((c::ast::Variable*)var,expr,assig->allocation);
   }
