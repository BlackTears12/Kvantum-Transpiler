...
namespace kvantum::parser
{
   ...
   void TypeChecker::checkModule(Module* mod)
   {
      ...
      symbols->pushSegment(apply(ITER_THROUGH(objt), std::function([this](ObjectType& t) -> std::pair<string,Type&> {
          return std::pair<string,Type&>(t.getName(), (Type&)t); 
      })));
   ...
   void TypeChecker::checkFunction(FunctionNode* node)
   {
      ...
      if (checkedFunctions.count(node) > 0 || node->hasTrait(FunctionNode::EXTERNAL_BUILTIN)) {
   ...
   void TypeChecker::checkObject(ObjectType* type)
   {
       ...
       symbols->pushSegment(apply(node->fields.begin(), node->fields.end(), std::function([](pair<string, Type&> p) { return p; })));
   ...
   any TypeChecker::visit(ArrayExpression* arr)
   {
      ...
      for (auto& e : arr->initializer){
          KVANTUM_VERIFY(visitExpression(e) == type,e->getType().getName() + " does not equal array type "+arr->getType().asArray().getType().getName());
      }
   ...
   any TypeChecker::visit(ArrayIndex* arr)
   {
       ...
       KVANTUM_VERIFY_ERROR(it == PrimitiveType::get(PrimitiveType::Integer),"cannot index with non integer type "+it.getName());
   ...
   void TypeChecker::visit(Assigment* assig)
   {
      ...
      if(!symbols->isDeclared(assig->variable->id) && !assig->variable->isField())
         assig->allocation = true;
   ...
   {
      auto type = getCurrentFunc()->returnType;
      ...
      if(ret->expr->exprtype == ExprType::FUNCTION_CALL && static_cast<FunctionCall*>(ret->expr)->fnode == this->getCurrentFunc())
          return;

      KVANTUM_VERIFY(value != Type::get("Void"), 
          "cannot return void");

      else KVANTUM_VERIFY((type == Type::get("Void") && !getCurrentFunc()->hasTrait(FunctionNode::EXPLICIT_TYPE)) || value == type,
          type.getName() + " is not same as " + value.getName()); 

      else getCurrentFunc()->returnType = value;
   }
   ...
   any TypeChecker::visit(FunctionCall* fcall)
   {
      ...
      if (identifier.isField() && identifier.getBaseType() == Type::get("Void")) {
          KVANTUM_VERIFY_ERROR(symbols->isDeclared(identifier.parentObj),"no object declared "+identifier.parentObj);
          identifier.setBaseType(symbols->get(identifier.parentObj));
      }

      KVANTUM_VERIFY_ERROR(mod->hasFunction(identifier), "no function named " + identifier.name + " with arguments: "+ identifier.getArgumentListStr());
      ...
      if (!fcall->fnode->hasTrait(FunctionNode::STATIC) && fcall->var->isField())
          fcall->arguments.insert(fcall->arguments.begin(), new Variable("self", fcall->var->as<FieldAccess*>()->base->getType()));

      ///verify the arguments size an type order
      KVANTUM_VERIFY(fcall->fnode->formalParams.size() == fcall->arguments.size(),
          "function " + fcall->fnode->getName() + " expects " + std::to_string(fcall->fnode->formalParams.size())
          + " arguments, but " + std::to_string(fcall->arguments.size()) + " were provided");
      ...
      for (int i = 0; i < std::min(fcall->fnode->formalParams.size(), fcall->arguments.size()); i++) {
          ...
          KVANTUM_VERIFY(e->getType() == param->type,
              e->getType().getName() + " does not equal expected " + param->getType().getName());
      }
      return &fcall->fnode->returnType;
   }
   ...
   Type& TypeChecker::handleVariable(Variable* var, bool assig)
   {
       ...
       if (var->isField()) {
           ...
           KVANTUM_VERIFY_RETURN(f->base->getType().isObject(), "cannot access a field of a non-object " + f->base->getType().getName(), KVANTUM_TYPE_ERROR);
           f->field->type = f->base->getType().asObject().getFieldType(f->field->id);
       }
       if (assig)
           return KVANTUM_TYPE_ERROR;
       if (var->getType() == PrimitiveType::get(PrimitiveType::Void)) {
           KVANTUM_VERIFY_RETURN(symbols->isDeclared(var->id),"variable not declared "+var->id,KVANTUM_TYPE_ERROR);
