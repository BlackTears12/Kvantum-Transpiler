...
namespace kvantum::parser
{
    ...
    void TypeChecker::checkModule(Module* mod)
    {
        ...
        symbols.pushSegment(apply(ITER_THROUGH(objt), function<pair<string, Type*>(ObjectType*)>([this](ObjectType* t) -> pair<string, Type*> {
    ...
    void TypeChecker::checkObject(ObjectType* type)
    {
        ...
        symbols.pushSegment(apply(node->fields.begin(), node->fields.end(), function<pair<string, Type*>(pair<string, Type*>)>
    ...
    any TypeChecker::visit(TakeReference* ref)
    {
        ...
        KVANTUM_VERIFY_ERROR(ref->baseExpr->exprtype == ExprType::VARIABLE, "cannot take reference from non-variable");
        return &ref->getType();
    }
    ...
    void TypeChecker::visit(Assigment* assig)
    {
        ...
        KVANTUM_VERIFY(assig->variable->isField() || assig->isDeclaration() || symbols.isDeclared(assig->variable->id),
                       "variable not declared "+assig->variable->id);
        KVANTUM_VERIFY(assig->variable->isField() || !assig->isDeclaration() || !symbols.isDeclaredLocal(assig->variable->id),
        ...
        KVANTUM_VERIFY(assig->variable->getType().isVoid() || assig->variable->getType() == value,
                       "expression type " + value.getName() + " does not equal specified type " + assig->variable->getType().getName());
