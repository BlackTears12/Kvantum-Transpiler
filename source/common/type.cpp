...
namespace kvantum
{
   ...
   template <typename T>
   std::optional<typename vector<T>::iterator> find(typename vector<T>::iterator begin,typename vector<T>::iterator end,T item){
      while(begin != end){
         if(*begin == item)
            return begin;
         begin++;
   ...
   PrimitiveType& Type::asPrimitive() { return static_cast<PrimitiveType&>(*this); }
   ObjectType& Type::asObject() { return static_cast<ObjectType&>(*this); }
   ArrayType& Type::asArray() { return static_cast<ArrayType&>(*this); }
   ReferenceType& Type::asReference() { return static_cast<ReferenceType&>(*this); }
   ...
   unsigned int ObjectType::getAllocSize() 
   {
       ...
       for (auto& e : node->fields) {
           ...
           else sz += Type::getPointerAllocSize();
       }
       return sz;
   }
   ...
   FunctionNode* ObjectType::getFunction(Variable* const var)
   {
      ...
      else if(hasField(var) && getFieldType(var->id).isObject() && getFieldType(var->id).asObject().hasFunction(var->as<FieldAccess*>()->field))
   ...
   {
       auto fields = apply(ITER_THROUGH(node->fields), std::function([](std::pair<string,Type*> p) {
           return p;
       }));
       if (parent) {
   ...
   {
       auto methods = apply(ITER_THROUGH(node->methods), std::function([](std::pair<string, FunctionNode*> p) {
           return p;
       }));
       if (parent) {
   ...
   /* ListType methods */
   ListType::ListType(Type& t) : ObjectType(new TypeNode("["+t.getName() + "]")),type(t)
   {
       ...
       /*
            ...
            modifies max_value
       */
       FunctionNode* reSize = new FunctionNode(getTypeID()+"_reSize",Type::get("Void"),{new Variable("new_size",Type::get("Int"))});
       ...
       reSize->ast = {
           ...
           */
           ...
           new Assigment(accField("_arr"),new ArrayAllocation(type,new Variable("new_size",Type::get("Int")))),
           new FunctionCall(new Variable("memcpy"),{accField("_arr"),new Variable("arr"),accField("size")}),
       ...
       */
       ...
       /*
            at Int -> T
       */
       FunctionNode* at = new FunctionNode(getTypeID() + "_at", type, {new Variable("index",Type::get("Int"))});
       ...
       */
       FunctionNode* append = new FunctionNode(getTypeID() + "_append",Type::get("Void"),{new Variable("item",type)});
       append->ast = {
           /*
       ...
       /*
            Constructor
       */
       FunctionNode* cctor = new FunctionNode(getTypeID() + "_new", *this, {new Variable("initializer",ArrayType::get(t)),new Variable("arr_size",Type::get("Int"))});
