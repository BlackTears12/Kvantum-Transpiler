...
namespace kvantum::parser
{
    ...
    */
    Statement* Parser::parseStatement()
    {
        ...
        while (!valid && !lexer->end()) {
            ...
            switch (token.type) {
                ...
                CASE(Token::LET,valid = parseAssigment(parseVariable(lexer->nextToken()))->setDeclaration(true));
    ...
    Assigment* Parser::parseAssigment(Variable* var)
    {
        ...
        if(lexer->lookAhead().type == Token::COLON){
            lexer->nextToken();
            assignTy = parseType().value_or(&Type::get("Void"));
        }
